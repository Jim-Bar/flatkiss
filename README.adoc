...
TIP: Several collisions can be added for a single sprite index. Every one of those will be checked to determine whether
an object collides with the sprite.

A group of collisions starts with the index on two bytes. Then the number of collisions making up the group on two
bytes. Then come the collisions concatenated together.
