...
with open('levels.txt', 'w') as level_file:
    ...
    while len(level_bytes) > 0:
        width, height, spriteset_index, animation_index = struct.unpack('HHHH', level_bytes[:4 * 2])
        tiles = struct.unpack('H' * width * height, level_bytes[4 * 2:4 * 2 + width * height * 2])
        ...
        level_bytes = level_bytes[4 * 2 + width * height * 2:]
