...
Collision::Collision(std::vector<PositionedEllipse> const PositionedEllipses, vector<PositionedRectangle> const PositionedRectangles) : PositionedEllipses(move(PositionedEllipses)), PositionedRectangles(move(PositionedRectangles)) {
...
unordered_map<uint16_t, Collision const> CollisionLoader::load(std::string const& FilePath) {
            ...
            switch (CollisionType) {
                case 0: // The collision is a positioned rectangle.
                    if (!RectanglesPerTileIndex.contains(TileIndex)) {
                        RectanglesPerTileIndex.emplace(TileIndex, vector<PositionedRectangle>{});
                    }
                    RectanglesPerTileIndex[TileIndex].emplace_back(Position, Rectangle{static_cast<uint8_t>(Stream.get()), static_cast<uint8_t>(Stream.get())});
                    break;
                case 1: // The collision is a positioned ellipse.
                    if (!EllipsesPerTileIndex.contains(TileIndex)) {
                        EllipsesPerTileIndex.emplace(TileIndex, vector<PositionedEllipse>{});
                    }
                    EllipsesPerTileIndex[TileIndex].emplace_back(Position, Ellipse{static_cast<uint8_t>(Stream.get()), static_cast<uint8_t>(Stream.get())});
                    break;
            }
        }
        Stream.close();
    ...
    // Secondly, once all the positioned rectangles / ellipses are known, the collisions can be built.
    unordered_map<uint16_t, Collision const> CollisionsPerTileIndex;
    for (auto TileIndex : AllTilesIndices) {
        CollisionsPerTileIndex.emplace(std::piecewise_construct, std::forward_as_tuple(TileIndex),
            std::forward_as_tuple(move(EllipsesPerTileIndex[TileIndex]), move(RectanglesPerTileIndex[TileIndex])));
    }
