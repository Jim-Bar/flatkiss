...
void Renderer::render(AnimationPlayer const& AnimationPlayer,
                      Level const& Level, Tileset const& Tileset,
                      size_t ViewPortX, size_t ViewPortY,
  ...
  renderLevel(AnimationPlayer, Level, Tileset, ViewPortX, ViewPortY,
              ViewPortSizeInPixels, Tick);
  renderCharacter(ViewPortX, ViewPortY, Tick, CharacterTexture, CharacterX,
                  CharacterY, CharacterSizeInPixels);
  SDL_RenderPresent(SDLRenderer);
}

void Renderer::renderCharacter(size_t ViewPortX, size_t ViewPortY, size_t Tick,
                               SDL_Texture* CharacterTexture, size_t CharacterX,
                               size_t CharacterY,
                               size_t CharacterSizeInPixels) const {
  SDL_Rect DestRect;
...
void Renderer::renderLevel(AnimationPlayer const& AnimationPlayer,
                           Level const& Level, Tileset const& Tileset,
                           size_t ViewPortX, size_t ViewPortY,
                           size_t ViewPortSizeInPixels, size_t Tick) const {
  for (size_t Y(ViewPortY / Tileset.tilesSize());
       Y <= (ViewPortY + ViewPortSizeInPixels) / Tileset.tilesSize(); Y++) {
    for (size_t X(ViewPortX / Tileset.tilesSize());
         X <= (ViewPortX + ViewPortSizeInPixels) / Tileset.tilesSize(); X++) {
      uint16_t TileIndex(
          AnimationPlayer.animatedTileIndexFor(Level.tileIndex(X, Y), Tick));
