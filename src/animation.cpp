...
Animation::Animation(std::vector<uint16_t> const& TileIndices, uint8_t Period,
                     uint8_t Duration)
    : TileIndices(std::move(TileIndices)), Period(Period), Duration(Duration) {}

uint8_t Animation::getDuration() const { return Duration; }

uint8_t Animation::getPeriod() const { return Period; }
