...
Collider::Collider(unordered_map<uint16_t, Solid>&& solids_per_tile_index)
    : solids_per_tile_index_{move(solids_per_tile_index)} {}

bool Collider::collide(PositionedSolid const& positioned_solid,
                       uint16_t tile_index,
                       Position const& tile_position) const {
  if (!solids_per_tile_index_.contains(tile_index)) {
    return false;
  }

  Solid const& solid{solids_per_tile_index_.at(tile_index)};

  return positioned_solid.collidesWith(PositionedSolid{tile_position, solid});
}

Solid const& Collider::zero() const { return solids_per_tile_index_.at(0); }
...
unordered_map<uint16_t, Solid> ColliderLoader::loadGroup(
   ...
   * solids can be built. */
  unordered_map<uint16_t, Solid> solids_per_tile_index;
  for (auto tile_index : all_tiles_indices) {
    solids_per_tile_index.emplace(
